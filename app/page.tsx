...
export default function DashboardPage() {
  const [activeTab, setActiveTab] = useState('analytics'); // Cambiado a analytics por defecto
  ...
  useEffect(() => {
    async function getInitialData() {
      ...
      if (user) {
        ...
        if (profileData) {
          // Aseguramos que todos empiecen en analytics
  ...
  if (!profile) return <div className="text-white p-10 bg-[#020617] min-h-screen flex items-center justify-center font-mono uppercase tracking-tighter">Acceso Denegado</div>;
  ...
  const tabs = profile.role === 'MANAGER' 
    ...
    : [
        { id: 'analytics', label: 'Dashboard', icon: BarChart3, color: 'text-blue-400', bg: 'bg-blue-600/20' }, // Ahora visible para todos
  ...
  return (
    <main className="relative min-h-screen w-full bg-[#020617] text-slate-200 overflow-x-hidden">
      ...
      {/* HEADER REDISEÑADO: FUTURISTA Y VIVO */}
          ...
          <div className="flex items-center gap-6 group">
            ...
            >
              <motion.div 
                ...
                className="absolute inset-0 border-2 border-dashed border-blue-500/30 rounded-2xl"
              />
              <motion.div 
                whileHover={{ rotate: 180 }}
                transition={{ type: "spring", stiffness: 260, damping: 20 }}
                className="h-11 w-11 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-700 flex items-center justify-center shadow-[0_0_20px_rgba(37,99,235,0.4)] border border-white/20 z-10"
              >
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-3">
                <h1 className="text-3xl font-black tracking-tighter flex items-center leading-none">
                  ...
                  <span className="relative ml-1 italic bg-clip-text text-transparent bg-gradient-to-r from-blue-400 via-cyan-300 to-blue-600 bg-[length:200%_auto] animate-gradient-x">
                    PORTAL
                  </span>
                </h1>
          ...
          <div className="flex items-center gap-6">
            ...
            <form action="/api/auth/signout" method="post">
              <motion.button 
                ...
                whileTap={{ scale: 0.95 }}
                className="p-3 rounded-xl bg-white/5 border border-white/10 hover:text-rose-400 transition-all shadow-inner"
              >
                <LogOut size={18} />
              </motion.button>
            </form>
        ...
        <aside className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-1">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`relative w-full flex items-center gap-3 px-5 py-4 rounded-2xl transition-all duration-300 group ${
                    activeTab === tab.id ? 'text-white' : 'text-slate-500 hover:bg-white/5'
                  }`}
                >
                  {activeTab === tab.id && (
                    <motion.div layoutId="pill" className={`absolute inset-0 rounded-2xl ${tab.bg} border border-white/10 shadow-[0_0_15px_rgba(255,255,255,0.05)]`} />
                  )}
                  <div className={`relative z-10 transition-transform ${activeTab === tab.id ? 'scale-110 ' + tab.color : 'group-hover:text-slate-300'}`}>
                    <tab.icon size={18} />
                  </div>
                  <span className="relative z-10 text-[10px] font-black uppercase tracking-wider text-left leading-tight flex-1">
                    {tab.label}
                  </span>
                  {tab.badge ? (
                    <span className="relative z-10 bg-rose-600 text-[9px] px-1.5 py-0.5 rounded-md font-bold">{tab.badge}</span>
                  ) : null}
                </button>
              ))}
            </div>
          ...
          <div className="p-6 rounded-[2rem] border border-white/10 bg-gradient-to-br from-blue-500/10 to-transparent backdrop-blur-xl relative overflow-hidden group">
            <div className="absolute -right-4 -top-4 opacity-10 group-hover:scale-125 transition-transform duration-700">
              <Fingerprint size={100} className="text-blue-400" />
            </div>
            <p className="text-[9px] font-black text-blue-400/60 uppercase tracking-[0.3em] mb-4">Auth Level</p>
            <h3 className="text-3xl font-black text-white italic tracking-tighter uppercase mb-6 drop-shadow-lg">
              {profile.role}
            </h3>
            <div className="space-y-2">
