...
begin
  ...
  if exists (
    select 1 from public.early_friday_requests conflict
  ...
  -- Preserve the explicit state machine and its audit trail.
  update public.early_friday_requests
  set status = 'APPROVED_BY_TEAM_LEADER', current_approval_level = 'PORTFOLIO'
  where id = req.id;

  update public.early_friday_requests
  set status = 'PENDING_PORTFOLIO'
  where id = req.id;
