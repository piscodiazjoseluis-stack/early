...
begin
  ...
  select t.leader_user_id, 'APPROVAL_REQUIRED', 'Solicitud corregida pendiente',
    'Una solicitud corregida requiere una nueva revisión.', req.id
  from public.teams t
  where t.id = membership.team_id and next_level = 'TEAM_LEADER'
  union all
  select ur.user_id, 'APPROVAL_REQUIRED', 'Solicitud corregida pendiente',
    'Una solicitud corregida requiere una nueva revisión.', req.id
  ...
  insert into public.notifications (user_id, type, title, body, request_id)
  values (actor, 'REQUEST_UPDATED', 'Solicitud reenviada',
    'Tu solicitud corregida volvió al flujo de aprobación.', req.id);
...
create or replace function public.resubmit_returned_request(
  target_request uuid,
  target_date date,
  start_time time,
  end_time time,
...
set search_path = ''
as $$
  select private.resubmit_returned_request_impl(
    target_request,
    target_date,
    start_time,
    end_time,
...
grant execute on function private.resubmit_returned_request_impl(uuid, date, time, time, text)
  to authenticated;
revoke execute on function public.resubmit_returned_request(uuid, date, time, time, text)
  from public, anon;
grant execute on function public.resubmit_returned_request(uuid, date, time, time, text)
  to authenticated;
